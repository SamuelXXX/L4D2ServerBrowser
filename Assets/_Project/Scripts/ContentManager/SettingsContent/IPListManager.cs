...
public class IPListManager : ShortLifeSingleton<IPListManager>
{
    ...
    #region Exposed API
    protected List<IPData> ipDataCache = new List<IPData>();
    ...
    void OnReceiveIPLibraryResponse(WebRequestAgent.WebResponseData data)
    {
        ...
        if (data.responseType == WebRequestAgent.ResponseDataType.Text)
        {
            ...
            ipDataCache.AddRange(StringToServerConnectInfos(data.text));
        }

        ipDataCache.AddRange(StringToServerConnectInfos(ReadLocalConfig()));
        contentReadyHandler?.Invoke(ipDataCache, serverQuerySucceed);
